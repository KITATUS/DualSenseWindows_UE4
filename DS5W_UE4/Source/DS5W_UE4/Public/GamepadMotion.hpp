...
namespace GamepadMotionHelpers
{
	...
	void Motion::Update(float inGyroX, float inGyroY, float inGyroZ, float inAccelX, float inAccelY, float inAccelZ, float gravityLength, float deltaTime)
	{
		...
		if (accelMagnitude > 0.0f)
		{
			...
			{
				gravCorrectionSpeed = gravityCorrectionStillSpeed + (gravityCorrectionShakySpeed - gravityCorrectionStillSpeed) * MathHelpers::clamp((Shakiness - gravityCorrectionShakinessMinThreshold) / (gravityCorrectionShakinessMaxThreshold - gravityCorrectionShakinessMinThreshold), 0.f, 1.f);
			}
			else
			{
			...
			if (gravCorrectionSpeed > gyroGravCorrectionLimit)
			{
				...
				{
					closeEnoughFactor = MathHelpers::clamp((gravToAccel.Length() - gravityCorrectionGyroMinThreshold) / (gravityCorrectionGyroMaxThreshold - gravityCorrectionGyroMinThreshold), 0.f, 1.f);
				}
				else
				{
			...
			const float errorAngle = acosf(MathHelpers::clamp(Vec(0.0f, -1.0f, 0.0f).Dot(gravityDirection), -1.f, 1.f));
			...
			accelDelta.z <= MinDeltaAccel.z * RecalibrateThreshold)
		{
			if (CalibrationData != nullptr && MinMaxWindow.NumSamples >= minStillnessSamples && MinMaxWindow.TimeSampled >= minStillnessCorrectionTime)
			{
				...
				if (doSensorFusion)
				{
					...
					if (crossLength > 0.f)
					{
						const float thisDotPrev = MathHelpers::clamp(thisNormal.Dot(previousNormal), -1.f, 1.f);
	...
	float accelX, float accelY, float accelZ, float deltaTime)
{
	...
	{
		AutoCalibration.AddSampleStillness(GamepadMotionHelpers::Vec(gyroX, gyroY, gyroZ), GamepadMotionHelpers::Vec(accelX, accelY, accelZ), deltaTime, static_cast<bool>(CurrentCalibrationMode & GamepadMotionHelpers::CalibrationMode::SensorFusion));
		AutoCalibration.NoSampleSensorFusion();
	}
	else
	{
